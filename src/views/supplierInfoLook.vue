<template>
  <div class="info-dialog">
    ...
    <div v-if="rukuShow">
      ...
      <el-form class="ruku">
        ...
        <el-form-item label="入库审查意见" label-width="160px">
          <el-input size="mini" disabled v-model="rukuForm.estimateHeadRemark" type="textarea"></el-input>
        </el-form-item>
      </el-form>
    </div>
    <div v-if="changeIf">
    ...
    <el-dialog title="评估详情" :visible.sync="pgShow" class="dialogPg">
      ...
      <div class="oneButton">
        <el-button type="primary" size="mini" @click="pgShow = false">关闭</el-button>
      </div>
    </el-dialog>
    <div v-if="zhkShow">
      <div class="informa" style="width: calc(100% - 20px); margin-left: 10px;">转黑库意见</div>
...
export default {
  ...
  mounted() {
    ...
    var typeIndex = hash.indexOf('headType=');
    if(index!= -1){
        this.headCode = hash.substring(index + 5, typeIndex-1);
    } else {
        this.headCode = '';
    }
    if(typeIndex!= -1){
        this.infoType = hash.substring(typeIndex + 9, hash.length);
    } else {
        this.infoType = '';
    }
    console.log(this.infoType, this.headCode);
    if(this.infoType == 'Stock'){
      this.rukuShow = true;
      this.zhkShow = false;
    } else {
      this.rukuShow = false;
...
// 点击分数
    ...
    getCodester() {
      ...
      this.api.getSupplierInfoLookList(params).then((res) => {
        var supplierInfosList = res.data.data.supplierInfosList;
        this.supplierCode = supplierInfosList[0].supplierCode;
        this.estimateHeadCode =
          supplierInfosList[0].estimateHeadCode || "";
        this.blacklistCode =
          supplierInfosList[0].blacklistCode || "";
        this.ruleForm = Object.assign(
          this.ruleForm,
          supplierInfosList[0]
        );
        this.supplierName =
          supplierInfosList[0].supplierName || "";
        // this.ruleForm = supplierInfosList[0];
        this.ruleForm.shareholderList = [];
        if(res.data.data.supplierPeopleList){
        ...
        this.scForm.supplierCostCenter =
          supplierInfosList[0].supplierCostCenter;
        // 营业期限
        this.times.push(
          supplierInfosList[0].licenseValidBegin,
          supplierInfosList[0].licenseValidEnd
        );
        // 城市回显
        if (supplierInfosList[0].supplierServiceregion) {
          var city = supplierInfosList[0].supplierServiceregion.split(
            ","
          );
        } else {
          var city = "";
        ...
        // 星际辉县
        this.ruleForm.supplierStar = supplierInfosList[0].supplierStar || '0';
        this.ruleForm.supplierStar = this.ruleForm.supplierStar + '星'; 
        // 供应商类别回显
        if (supplierInfosList[0].supplierSort) {
          var lb = supplierInfosList[0].supplierSort.split(",");
          if (lb.length > 0) {
        ...
        //
        if (supplierInfosList[0].estimateScore) {
          this.rukuForm.supplierScore =
            supplierInfosList[0].estimateScore;
        }
        if (supplierInfosList[0].estimateHeadRemark) {
          this.rukuForm.estimateHeadRemark =
            supplierInfosList[0].estimateHeadRemark;
        }
        if (this.getCodesterType == "Blacklist") {
          this.zztForm.supplierOption =
            supplierInfosList[0].blacklistAddReason || "";
        }
        if (this.changeIf) {
